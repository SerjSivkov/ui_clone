...
2. (Опционально) в **Настройки** укажите API key и vision-модель
   (`gpt-4o-mini` или совместимый endpoint). Либо выберите провайдер
   **Локально (без облака)** — скриншоты не уходят в сеть, палитра HEX
   собирается на устройстве.
...
Как внести вклад — в [CONTRIBUTING.md](CONTRIBUTING.md).
